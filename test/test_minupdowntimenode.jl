...
function create_system(line)
    ...
    links = [
        Direct("grid-line", grid, line),
        Direct("line-demand", line, demand),
    ]
    case = Dict(:T => T, :nodes => nodes, :products => [Power, Product],
        :links => links)
...
@testset "check-cyclic-sequence" begin
    ...
    for min_up_time in 2:8
        for min_down_time in 2:8
