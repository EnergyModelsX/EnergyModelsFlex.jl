...
ReserveUp = EMB.RefSink(
    ...
    Dict(:surplus => FixedProfile(0), :deficit => FixedProfile(0.12)), # NOK / kW / h - Price for not delivering maximum reserve capcity / price for providing reserve up
...
ReserveDown = EMB.RefSink(
    ...
    Dict(:surplus => FixedProfile(0), :deficit => FixedProfile(0.12)), # NOK / kW / h - Price for not delivering maximum reserve capcity / price for providing reserve down
...
battery = BatteryStorage(
    ...
    Dict(power => 1),
    Dict(power => 1, power_reserve_up => 1, power_reserve_down => 1),
    EMB.Data[],
)
